...
import { verifyNewEmail } from "@/lib/store/features/auth/auth-slice";
...
const VerifyNewEmailComp = ({ token }: Props) => {
  ...
  useEffect(() => {
    const activateUserAccount = async () => {
      if (token) {
        try {
          await dispatch(verifyNewEmail(token)).unwrap();
      ...
      <div className="p-6 border-b">
        ...
        <p className="text-sm text-gray-600 mt-1">
          {activationStatus === "pending" &&
            "Your new email is being verified..."}
          ...
          <div className="flex flex-col items-center space-y-4">
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-gray-600">
                Your new email has been successfully verified.
              </p>
            </div>
          </div>
        )}
...
export default VerifyNewEmailComp;
